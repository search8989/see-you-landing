...
(function () {
  ...
  var EU = ['польща','німеччина','чехія','словаччина','австрія','угорщина','румунія','болгарія',
            'іспанія','португалія','італія','франція','велика британія','нідерланди','бельгія',
            'швеція','норвегія','данія','фінляндія','естонія','латвія','литва','молдова',
            'ірландія','швейцарія','греція','хорватія','словенія','сербія'];
  var AS = ['туреччина','грузія','оае','ізраїль','таїланд','японія','південна корея','китай',
            'індія','сінгапур','узбекистан'];

  function regionOf(country) {
    var c = (country || '').trim().toLowerCase();
    if (!c || c === 'україна') return 'ua';
    if (EU.indexOf(c) > -1) return 'eu';
    if (AS.indexOf(c) > -1) return 'as';
    return 'eu';
  }
